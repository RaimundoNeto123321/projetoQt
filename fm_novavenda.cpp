...
void fm_novavenda::on_txt_codproduto_returnPressed()
{
    ...
    if(query.exec()){
        ...
        if(query.value(0).toString()!=""){
            ...
            QMessageBox::warning(this,"ERRO","Produto não encontrado","OK");
        }
        resetaCampos();
    }else{
        QMessageBox::warning(this,"ERRO","Erro ao inserir novo produto","OK");
    }
}
...
void fm_novavenda::on_btn_excluirproduto_clicked()
{
    if(ui->tw_listaprodutos->currentColumn()!=-1){
        ...
        QMessageBox::warning(this,"ERRO","Selecione um produto primeiro","OK");
    }
}
...
void fm_novavenda::on_btn_finalizarvenda_clicked()
{
    if(ui->tw_listaprodutos->rowCount()>0){
        ...
        if(!query.exec()){
            QMessageBox::warning(this,"ERRO","Erro ao registrar nova venda","OK");
        }else{
            ...
            while(linha<totalLinhas){
                ...
                query.prepare("insert into tb_produtosVendas (id_venda,produto,qtde,valor_un,valot_total)values("+QString::number(idVenda)+",'"+prod+"',"+qtde+","+valUn+","+valTot+")");
                query.exec();
                linha++;
            }

            QMessageBox::information(this,"Venda Concluída","msgFimVenda","OK");
            resetaCampos();
            removerLinhas(ui->tw_listaprodutos);
            ui->lb_totalvenda->setText("R$ 0.00");
        }
    }else{
        QMessageBox::warning(this,"ERRO","Não existem produtos nesta venda\nPrimeiro adicione um produto","OK");
    }
}
