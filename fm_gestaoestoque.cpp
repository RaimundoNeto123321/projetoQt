    ...
    ui(new Ui::fm_gestaoEstoque)
{
    ui->setupUi(this);
    if(!con.abrir()){
        QMessageBox::warning(this,"ERRO","Erro ao abrir banco de dados","OK");
    }else{
        QSqlQuery query;
        ...
        if(query.exec()){
            ...
            QMessageBox::warning(this,"ERRO","Erro ao listar produtos","OK");
        }
    }
...
void fm_gestaoEstoque::on_btn_gravarnovoproduto_clicked()
{
    ...
    QString produto=ui->txt_descicaoproduto->text();
    ...
    if(!query.exec()){
        QMessageBox::warning(this,"ERRO","Erro ao inserir produto","OK");
    }else{
        QMessageBox::information(this,"GRAVADO","Produto inserido com sucesso","OK");
        ui->txt_codigoproduto->clear();
        ui->txt_descicaoproduto->clear();
        ui->txt_fornecedor->clear();
        ui->txt_valorvenda->clear();
...
void fm_gestaoEstoque::on_btn_ge_gravar_clicked()
{
    if(ui->txt_ge_codigoproduto->text()==""){
        QMessageBox::warning(this,"ERRO","Primeiro selecione um registro","OK");
    }else{
        int id=ui->tw_ge_produtos->item(ui->tw_ge_produtos->currentRow(),0)->text().toInt();
        QString aux;
        ...
        if(query.exec()){
            ...
            QMessageBox::information(this,"Atualizado","Produto atualizado com sucesso","OK");
        }else{
            QMessageBox::warning(this,"ERRO","Erro ao atualizar produto","OK");
        }
    ...
    if(ui->txt_ge_codigoproduto->text()==""){
        ...
        if(opc==QMessageBox::Yes){
            ...
            if(query.exec()){
                ...
                QMessageBox::information(this,"DELETADO","Produto foi deletado","OK");
            }else{
                QMessageBox::warning(this,"ERRO","Erro ao excluir produto","OK");
            }
        }
    }
}
...
void fm_gestaoEstoque::on_btn_ge_filtrar_clicked()
{
    ...
    if(ui->txt_ge_filtar->text()==""){
        ...
        if(ui->rb_ge_codigo->isChecked()){
            busca="select id_produto,produto from tb_produtos where id_produto="+ui->txt_ge_filtar->text()+" order by id_produto";
        }else{
            busca="select id_produto,produto from tb_produtos where produto like '%"+ui->txt_ge_filtar->text()+"%' order by produto";
        }
    }
    int contlinhas=0;
    QSqlQuery query;
    ...
    if(query.exec()){
        ...
        QMessageBox::warning(this,"ERRO","Erro ao listar produto","OK");
    }
    ui->txt_ge_filtar->clear();
    ui->txt_ge_filtar->setFocus();
}
