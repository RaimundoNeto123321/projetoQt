...
void fm_gestaoVendas::on_tw_listaVendas_itemSelectionChanged()
{
   ...
   int contLinhas=0;
   QSqlQuery query;
   query.prepare("select id_movimentacao,produto,qtde,valor_un,valot_total from tb_produtosVendas where id_venda="+ui->tw_listaVendas->item(ui->tw_listaVendas->currentRow(),0)->text());
   query.exec();
   query.first();
   do{
...
void fm_gestaoVendas::on_btn_relatorioPDF_clicked()
{
    ...
    painter.drawPixmap(25,25,QPixmap(":/imgs/teste.png"));

    painter.drawText(25,100,"TESTE DE PDF");
